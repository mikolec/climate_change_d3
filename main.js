...
var divSelection = d3.select("#viz").selectAll("div");
...
divSelection
  ...
  .append("div")
  .attr("class", "bar")
  .style("width", (d, i) => d.deaths * 8 + "px")
  .on("mouseover", (d, i) => toggleClass(i + 1, true))
  .on("mouseout", (d, i) => toggleClass(i + 1, false));

var listSelection = d3.select("#legend").selectAll("li");

listSelection
  .data(climate_daly_data)
  .enter()
  .append("li")
  .text((d) => `${d.region}: ${d.deaths} deaths`)
  .on("mouseover", (d, i) => toggleClass(i + 1, true))
  .on("mouseout", (d, i) => toggleClass(i + 1, false));
